...
export default function App() {
  ...
  async function handleLogout() { await supabase.auth.signOut(); setSideMenu(null); }
  ...
  async function handleDelete(id: string) { await supabase.from("records").delete().eq("id", id); fetchRecords(); }
  ...
  const statsCurrentMonth = useMemo(() => {
    const entries = records.filter(r => { const d = parseDate(r.date); return d.getFullYear() === currentYear && d.getMonth() === currentMonth; });
  ...
  const chartData = useMemo(() => {
    const days: any = {};
    records.forEach(r => {
      const d = parseDate(r.date); const day = d.getDate();
  ...
  const recentRecords = useMemo(() => records.filter(r => parseDate(r.date).getFullYear()===currentYear).slice(0,30), [records, currentYear]);
  const archivioYears = useMemo(() => { const ys = new Set(records.map(r => parseDate(r.date).getFullYear())); return [...ys].sort((a:any,b:any)=>b-a); }, [records]);
  const archivioData = useMemo(() => {
    if (!archivioYear) return null;
    const yr = Number(archivioYear);
    return MONTHS.map((_, mi) => {
      const entries = records.filter(r => { const d = parseDate(r.date); return d.getFullYear()===yr && d.getMonth()===mi; });
  ...
  const confrontoData = useMemo(() => {
    ...
    return MONTHS_SHORT.map((name,mi) => {
      const sum = (yr:string) => records.filter(r=>{ const d=parseDate(r.date); return d.getFullYear()===Number(yr)&&d.getMonth()===mi; }).reduce((a,r)=>a+Number(r.incasso),0);
  ...
  function handlePreview() {
    ...
    const parsed = lines.map(line => { const parts = line.trim().split(/[\s\t]+/); return { giorno: parseInt(parts[0]), coperti: parseInt(parts[1]), incasso: parseFloat(parts.slice(2).join("").replace(/[€]/g,"").replace(",",".")) }; }).filter(p=>!isNaN(p.giorno)&&!isNaN(p.coperti)&&!isNaN(p.incasso));
  ...
  if (loadingAuth) return <div style={{ minHeight:"100vh", background:BG, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"system-ui,sans-serif" }}><div style={{ textAlign:"center", color:MUTED }}><div style={{ fontSize:40, marginBottom:12 }}>🍽️</div><div>Caricamento...</div></div></div>;
  ...
  if (screen==="login"||screen==="register") return (
    <div style={{ minHeight:"100vh", background:BG, display:"flex", alignItems:"center", justifyContent:"center", padding:20, fontFamily:"system-ui,sans-serif" }}>
      ...
      <div style={{ background:CARD, borderRadius:20, padding:"40px 32px", width:"100%", maxWidth:400, boxShadow:"0 4px 24px rgba(0,0,0,0.08)" }}>
        <div style={{ textAlign:"center", marginBottom:20 }}><div style={{ display:"inline-flex", background:ORANGE, borderRadius:18, width:72, height:72, alignItems:"center", justifyContent:"center", fontSize:32, color:"#fff" }}>€</div></div>
        ...
        <button onClick={screen==="login"?handleLogin:handleRegister} disabled={loginLoading} style={{ width:"100%", background:loginLoading?"#d4a090":ORANGE, color:"#fff", border:"none", borderRadius:12, padding:14, fontSize:16, fontWeight:600, cursor:"pointer", marginBottom:16 }}>{loginLoading?"...":screen==="login"?"Accedi":"Crea Account"}</button>
        <div style={{ borderTop:"1px solid #eee", paddingTop:16, textAlign:"center" }}>
          ...
          <button onClick={()=>{ setScreen(screen==="login"?"register":"login"); setLoginError(""); }} style={{ width:"100%", background:"none", border:"1.5px solid #e0d8cc", borderRadius:12, padding:12, fontSize:15, fontWeight:600, cursor:"pointer", color:TEXT }}>{screen==="login"?"Crea Nuovo Account":"Accedi"}</button>
        </div>
      </div>
    </div>
  );
          ...
          <div style={{ width:"90%", maxWidth:440, background:BG, overflowY:"auto" }}>
            ...
            <div style={{ display:"flex", background:"#e8e0d4", borderRadius:12, margin:"16px 16px 0", padding:3 }}>
              {["archivio","confronto","importa","account"].map(t=>(
                <button key={t} onClick={()=>setSideMenu(t)} style={{ flex:1, padding:"8px 2px", border:"none", borderRadius:9, cursor:"pointer", fontSize:12, fontWeight:600, background:sideMenu===t?CARD:"transparent", color:sideMenu===t?TEXT:MUTED }}>{t.charAt(0).toUpperCase()+t.slice(1)}</button>
              ))}
            </div>
                ...
                <div>
                  ...
                  {archivioData&&archivioData.filter((m:any)=>m.giorni>0).map((m:any,i:number)=>(
                    <div key={i} style={{ background:CARD, borderRadius:12, padding:"14px 16px", marginBottom:10 }}>
                      <div style={{ fontWeight:700, fontSize:15, marginBottom:10 }}>{MONTHS[archivioData!.indexOf(m)]}</div>
                ...
                <div>
                  ...
                  {confrontoData&&(
                    <div style={{ background:CARD, borderRadius:12, padding:16 }}>
                      <div style={{ fontWeight:700, marginBottom:12 }}>Confronto {confAnno1} vs {confAnno2}</div>
